angular.module('pimServices', []).
	...
	factory('Publication', function($resource) {
		return $resource('/publications/folders/:folder/files/:id', {folder : '', id : '@_id'}, { 
			update : {
				method : 'PUT' // this method issues a PUT request
		    }
		});
	}).
	service('publicationService', ['$state', '$http', '$q', function($state, $http, $q) {
		var me = this;
//		me.folders = [
...
//   		];
		...
		me.findItem = function(itemId) {
			var foundedItem = null;
			$.each(me.folders, function() {
				foundedItem = $.grep(this.items || [], function(item) {
					return item.id == itemId;
				})[0];
				if (foundedItem) {
					foundedItem.folderId = this.id;
				}
				return !foundedItem;
			});
			return foundedItem;
		...
		me.findFolder = function(folderId) {
			return $.grep(me.folders, function(folder) {
		...
		me.stateChanged = function() {
			me.selectedItem = me.findItem($state.params.itemId);
			me.selectedFolder = me.findFolder($state.params.folderId);
		};
	}]);