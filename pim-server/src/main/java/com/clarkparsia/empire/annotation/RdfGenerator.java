...
public final class RdfGenerator {
    ...
    private static <T> Class<T> determineClass(Class<T> theOrigClass, T theObj, DataSource theSource, boolean instanceClass) throws InvalidRdfException, DataSourceException {
...
//		final Collection<Value> aTypes = DataSourceUtil.getValues(theSource, EmpireUtil.asResource(EmpireUtil.asSupportsRdfId(theObj)), RDF.TYPE);
		...
		final Collection<Value> aTypes = Collections.singletonList(new URIImpl(StringUtils.substringBeforeLast(rdfId, ":")));
		// ======================================== end speedup PATCH ========================================
			...
			if (theValue instanceof Literal) {
				...
				try {
					...
					if (aClass.isAssignableFrom(java.net.URI.class)) {
						...
						Object proxyOrDbObject = getProxyOrDbObject(mAccessor, aClass, java.net.URI.create(aURI.toString()), mSource);
						return proxyOrDbObject;
					}
				}
				catch (Exception e) {
