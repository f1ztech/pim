...
public final class GoogleAuthHelper {
	...
	private static final Collection<String> SCOPE = Arrays
			.asList("https://www.googleapis.com/auth/userinfo.profile;https://www.googleapis.com/auth/userinfo.email;https://www.googleapis.com/auth/gmail.modify".split(";"));
	...
	public GoogleAuthHelper(String userId) {
		...
		try {
			...
			flow = new GoogleAuthorizationCodeFlow.Builder(HTTP_TRANSPORT, JSON_FACTORY, CLIENT_ID, CLIENT_SECRET, SCOPE) // .setApprovalPrompt("force")
					.setDataStoreFactory(data_store_factory).setApprovalPrompt("force").setAccessType("offline").build();
	...
	public void getCredential(String authCode) {
		try {
			...
			Credential credential = createCredentialWithRefreshToken(HTTP_TRANSPORT, JSON_FACTORY, CLIENT_ID, CLIENT_SECRET, response);
		} catch (IOException ex) {
			Logger.getLogger(GoogleAuthHelper.class.getName()).log(Level.SEVERE, null, ex);
		}
	...
	public static GoogleCredential createCredentialWithRefreshToken(HttpTransport transport, JsonFactory jsonFactory, String clientId, String clientSecret,
			TokenResponse tokenResponse) {
		return new GoogleCredential.Builder().setTransport(transport).setJsonFactory(jsonFactory).setClientSecrets(clientId, clientSecret).build()
	...
	public String setMessageLabel(String messageId, String labelName) {
		try {
			...
			mods = new ModifyMessageRequest().setRemoveLabelIds(labelsToRemove).setAddLabelIds(labelsToAdd);
	...
	public Map getBareGmailMessageDetails(String messageId) {
		...
		try {
			...
			for (MessagePartHeader header : headers) {
				if (header.getName().equals("From") || header.getName().equals("Date") || header.getName().equals("Subject") || header.getName().equals("To")
	 ...
	 * @throws IOException
	 */
	public String sendMessage(String to, String from, String subject, String body) throws MessagingException, IOException {
